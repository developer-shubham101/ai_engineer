...
async def run_benchmark(top_k: int = 3) -> None:
    ...
    for qa in QA_PAIRS:
        ...
        results = await vector_store.search_documents(question, top_k=top_k)
        latency_ms = (time.perf_counter() - t0) * 1000
        latencies.append(latency_ms)

        # Recall@k: at least one relevant doc in top-k
        ...
        print(f"  {status} [{latency_ms:5.1f}ms] {question[:55]}")
