...
export default function RAGChat(){
  const [apiKey, setApiKey] = useState(
    localStorage.getItem("api_key") || "key-guest-1"
  );
  ...
  useEffect(()=>{ localStorage.setItem('x-session-id', sessionId) }, [sessionId])
  useEffect(()=>{
  ...
  async function startSession(){
    if (!apiKey) return addToast('API key required', 'warning', 'Auth')
    try {
      const res = await fetch(BASE_API_URL + '/api/rag/session/start', {
        method: "POST",
  ...
  async function endSession(){
    if (!apiKey) return addToast('API key required', 'warning', 'Auth')
    if (!sessionId) return addToast('No session to end', 'warning')
    try {
      const res = await fetch(BASE_API_URL + '/api/rag/session/end', {
        method: "POST",
        headers: { "Content-Type": "application/json", "X-API-Key": apiKey, "X-Session-ID": sessionId },
      });
      if (!res.ok){ const txt = await res.text().catch(()=>res.statusText); throw new Error(`Server error: ${res.status} - ${txt}`) }
      setSessionId('');
      ...
      <div className="d-flex justify-content-between align-items-center mb-2">
        ...
        <div className="d-flex gap-2 align-items-center">
          <div className="input-group input-group-sm">
            ...
            <button
              className={`btn ${
                rememberKey ? "btn-success" : "btn-outline-secondary"
              }`}
              onClick={() => setRememberKey((r) => !r)}
            >
              🔒
            </button>
          </div>
          ...
          <div className="card h-100">
            <div className="card-body d-flex flex-column">
              <div className="d-flex justify-content-between mb-2">
                <h5>Chat</h5>
                <small className="text-muted">Session: local</small>
              </div>
              <div
                ref={messagesRef}
                style={{
